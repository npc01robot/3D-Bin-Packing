...
class UCT(MCTS):
    def __init__(self, game: Game, allow_transpositions: bool = True, training: bool = True, c: float = 2 ** 0.5):
        super().__init__(game, allow_transpositions, training)
        self.game = game
    ...
    def selection(self, node: UCTNode) -> list[Node]:
        return super().selection(node)

    def expansion(self, path: List[UCTNode]) -> list[Node]:
        return super().expansion(path)

    def simulation(self, path: List[UCTNode]) -> list[Node]:
