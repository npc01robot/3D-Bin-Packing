...
from ability.py3dbp.spec.bin import Bin
from ability.py3dbp.spec.item import Item
from ability.py3dbp.utils.auxiliary_methods import (
    set2Decimal,
    calculate_standard_deviation,
)
from ability.py3dbp.utils.constants import (
    Axis,
...
class Packer:
    ...
    def packBin(self, item: Item, bin: Bin, max_width, max_height, max_depth):
        ...
        if not bin.items:
            return bin.putItemV2(item, [item.position],)
        # 正轴方向摆放
    ...
    def find_box(
        ...
        box = Bin("box", (0, 0, 0), max_weight=99999, put_type=0,number_of_decimals=number_of_decimals)
        for item in self.items:
