...
class OpenLoopNode(Node):
    def __init__(self, player: int, state: str, action: Optional[int] = None, prev_node: Optional[OpenLoopNode] = None):
        super().__init__(player, state, prev_node)
...
class OpenLoopMCTS(MCTS):
    ...
    def selection(self, node: OpenLoopNode) -> list[Node]:
